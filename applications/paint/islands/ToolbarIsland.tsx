...
import type { CanvasExportFormat, DrawingState, FileValidationOptions } from "../types/paint.ts";
import { AlertCircle, Download, Image, LogIn, Trash2, Upload } from "lucide-preact";
...
export default function ToolbarIsland({
  ...
  useEffect(() => {
    ...
    globalThis.dispatchEvent(new CustomEvent("paintColorChange", { detail: selectedColor }));
  ...
  const handleColorSelect = (color: string) => {
    ...
    globalThis.dispatchEvent(new CustomEvent("paintColorChange", { detail: color }));
  ...
  const handleWidthChange = (event: Event) => {
    ...
    globalThis.dispatchEvent(new CustomEvent("paintWidthChange", { detail: width }));
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        if (dataUrl) {
          ...
          console.log(
            "🖼️ Attempting to insert image, global function available:",
            !!(window as any).__paintCanvasImageInsert,
          );
      ...
      if (isUserAuthenticated && saveToServer) {
        try {
          ...
          if (getDrawingState && typeof getDrawingState === "function") {
            ...
            setSaveError(errorMessage);
            showWarning(
              "Local save completed, but server save failed: Drawing state not available",
            );
          }
        } catch (serverError) {
  ...
  const handleClearConfirm = async () => {
    ...
    try {
      ...
      showError(
        `Failed to clear canvas: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
      setIsClearing(false);
    ...
    <div class="space-y-4 sm:space-y-6">
      ...
      <div class="space-y-3">
        ...
        <button
          ...
          class={`btn btn-outline btn-sm w-full transition-all duration-200 ${
            isUploading ? "loading" : "hover:btn-primary"
          }`}
      ...
      <div class="space-y-3">
            ...
            : (
              <>
                ...
                <span class="hidden sm:inline">
                  {isUserAuthenticated && saveToServer
                    ? "Save & Download + Server"
                    : "Save & Download"}
                </span>
        ...
        <details class="text-xs text-base-content/50">
          ...
          <div class="mt-2 space-y-1 pl-2 border-l-2 border-primary/30">
            {saveFormat === "png" && <div>PNG: Best quality, larger file size</div>}
            {saveFormat === "jpeg" && <div>JPEG: Smaller file, slight quality loss</div>}
            {saveFormat === "webp" && <div>WebP: Modern format, good compression</div>}
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div class="bg-base-100 rounded-lg p-6 max-w-sm mx-4 shadow-xl">
            ...
            <p class="text-base-content/70 mb-6">
              This will permanently remove all drawings and images from the canvas. This action
              cannot be undone.
            </p>
