...
func GetPricings(w http.ResponseWriter, r *http.Request) {
	pricings, err := models.GetAllPricings(database.DB)
	if err != nil {
...
func GetPricing(w http.ResponseWriter, r *http.Request) {
	...
	if id, err := uuid.Parse(idOrName); err == nil {
		// It's a UUID
		pricing, err = models.GetPricingByID(database.DB, id)
	} else {
		// Try as name
		pricing, err = models.GetPricingByName(database.DB, idOrName)
	}
...
func GetPricingWithVariables(w http.ResponseWriter, r *http.Request) {
	...
	pricing, err := models.GetPricingByName(database.DB, name)
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to fetch pricing")
		return
	}
	...
	requiredVars, err := models.GetRequiredVariablesForPricing(database.DB, pricing)
	if err != nil {
...
func CreatePricing(w http.ResponseWriter, r *http.Request) {
	...
	if err := models.CreatePricing(database.DB, &pricing); err != nil {
...
func UpdatePricing(w http.ResponseWriter, r *http.Request) {
	...
	if err := models.UpdatePricing(database.DB, id, &pricing); err != nil {
...
func DeletePricing(w http.ResponseWriter, r *http.Request) {
	...
	if err := models.DeletePricing(database.DB, id); err != nil {
		if err == models.ErrNotFound {
			respondWithError(w, http.StatusNotFound, "Pricing not found")
			return
		}
