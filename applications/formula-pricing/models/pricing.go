...
func GetAllPricings(db *sql.DB) ([]Pricing, error) {
	query := `
...
func GetPricingByID(db *sql.DB, id uuid.UUID) (*Pricing, error) {
	query := `
		SELECT id, name, description, pricing, created_at, updated_at
		FROM formulapricing.pricing
		WHERE id = $1
	...
	var p Pricing
	var pricingJSON []byte
	err := db.QueryRow(query, id).Scan(&p.ID, &p.Name, &p.Description, &pricingJSON, &p.CreatedAt, &p.UpdatedAt)
...
func GetPricingByName(db *sql.DB, name string) (*Pricing, error) {
	query := `
		SELECT id, name, description, pricing, created_at, updated_at
		FROM formulapricing.pricing
		WHERE name = $1
	...
	var p Pricing
	var pricingJSON []byte
	err := db.QueryRow(query, name).Scan(&p.ID, &p.Name, &p.Description, &pricingJSON, &p.CreatedAt, &p.UpdatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
...
func CreatePricing(db *sql.DB, p *Pricing) error {
	...
	err = db.QueryRow(query, p.Name, p.Description, pricingJSON).
...
func UpdatePricing(db *sql.DB, id uuid.UUID, p *Pricing) error {
	pricingJSON, err := json.Marshal(p.Pricing)
	if err != nil {
		return err
	}
	...
	err = db.QueryRow(query, id, p.Name, p.Description, pricingJSON).
		Scan(&p.UpdatedAt)
	if err == sql.ErrNoRows {
		return ErrNotFound
	}
...
func DeletePricing(db *sql.DB, id uuid.UUID) error {
...
func GetRequiredVariablesForPricing(db *sql.DB, pricing *Pricing) ([]Variable, error) {
	if pricing == nil {
		return nil, nil
	}
	...
	for _, rule := range pricing.Pricing {
		...
		if rule.Condition != "" && rule.Condition != "always" {
			condition, err := GetConditionByName(db, rule.Condition)
		...
		if rule.Calculation != "" {
			calculation, err := GetCalculationByName(db, rule.Calculation)
			if err == nil && calculation != nil {
				...
				for _, elem := range calculation.Calculation {
					...
					if !isOperator(elem) && !isLiteral(elem) {
						...
						if v, err := GetVariableByName(db, elem); err == nil && v != nil {
							variableNames[elem] = true
						} else if c, err := GetCalculationByName(db, elem); err == nil && c != nil {
	...
	for name := range variableNames {
		if v, err := GetVariableByName(db, name); err == nil && v != nil {
