...
func LoginPage(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		go func() {
			...
			models.CreateLog(database.DB, loginLog)
		}()
...
func DashboardLogs(w http.ResponseWriter, r *http.Request) {
	...
	// Get recent logs
	logs, err := models.GetRecentLogs(database.DB, 100)
	if err != nil {
		logs = []models.Log{}
	}

	// Get log stats
	stats, err := models.GetLogStats(database.DB)
	if err != nil {
...
func getCollectionInfo() []CollectionInfo {
	...
	database.DB.QueryRow("SELECT COUNT(*) FROM formulapricing.variables").Scan(&variableCount)
	database.DB.QueryRow("SELECT COUNT(*) FROM formulapricing.calculations").Scan(&calculationCount)
	database.DB.QueryRow("SELECT COUNT(*) FROM formulapricing.conditions").Scan(&conditionCount)
	database.DB.QueryRow("SELECT COUNT(*) FROM formulapricing.pricing").Scan(&pricingCount)
...
func VariablesPage(w http.ResponseWriter, r *http.Request) {
	...
	variables, _ := models.GetAllVariables(database.DB)
...
func CalculationsPage(w http.ResponseWriter, r *http.Request) {
	...
	calculations, _ := models.GetAllCalculations(database.DB)
...
func ConditionsPage(w http.ResponseWriter, r *http.Request) {
	...
	conditions, _ := models.GetAllConditions(database.DB)
...
func PricingPage(w http.ResponseWriter, r *http.Request) {
	...
	pricings, _ := models.GetAllPricings(database.DB)
...
func UsersPage(w http.ResponseWriter, r *http.Request) {
	...
	users, _ := models.GetAllUsers(database.DB)
