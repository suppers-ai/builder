...
async function extractComponentData(filePath: string): Promise<ExtractedComponentData | null> {
  try {
    ...
    const category = filePath.includes("/action/")
      ? "Actions"
      : filePath.includes("/display/")
      ? "Display"
    ...
    const description = descriptionMatch
      ? descriptionMatch[1]
      : `${title} component for user interfaces`;
...
async function convertComponentRoutes() {
  ...
  for (const filePath of componentFiles) {
    ...
    try {
      ...
      console.log(
        `   ✅ Generated: ${componentName}.examples.md (${data.examples.length} examples)`,
      );
      convertedCount++;
    } catch (error) {
