...
import type { EmailMessage, NotificationMessage, NotificationResult } from "../types.ts";
  ...
  async getDeliveryStatus(notificationId: string): Promise<any> {
    try {
      const response = await fetch(
        `https://onesignal.com/api/v1/notifications/${notificationId}?app_id=${this.config.appId}`,
        {
          headers: {
            "Authorization": `Basic ${this.config.apiKey}`,
          },
