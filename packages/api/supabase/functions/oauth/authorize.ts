...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      return handleAuthorizeRequest(url);
...
async function handleAuthorizeRequest(url: URL) {
  ...
  if (!client) {
    ...
    client = fallbackClients.get(clientId);
    if (!client) {
...
async function handleTokenRequest(req: Request) {
  ...
  if (!client) {
    // Fallback to hardcoded clients for development
    client = fallbackClients.get(clientId as string);
    if (!client) {
      return createErrorResponse("invalid_client", "Unknown client_id");
    }
  }
