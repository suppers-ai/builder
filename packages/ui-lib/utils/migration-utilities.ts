  ...
  updatedContent = updatedContent.replace(classRegex, (match, classString) => {
    ...
    classes.forEach((className: string) => {
      ...
      if (mapping) {
        ...
        if (mapping.breaking) {
          warnings.push(
            `Breaking change: ${mapping.from} → ${mapping.to}${
...
export async function migrateDirectory(
  ...
  const summary =
    `Migration complete: ${result.processedFiles}/${result.totalFiles} files processed, ${totalChanges} total changes, ${result.errors.length} errors`;
...
export async function getMigrationStats(
  ...
  mostCommonClasses: Array<
    { className: string; count: number; type: "daisyui" | "tailwind" | "unknown" }
  >;
      ...
      while ((match = classRegex.exec(content)) !== null) {
        ...
        classes.forEach((className) => {
          ...
          if (daisyuiClassNames.has(className)) {
            ...
            className.startsWith("btn-") || className.startsWith("input-") ||
