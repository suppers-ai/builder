...
import { EntitiesApiClient } from "../../lib/api-client/places/entities-api.ts";
import { ProductsApiClient } from "../../lib/api-client/products/products-api.ts";
...
export default function EntitiesTab({ user }: EntitiesTabProps) {
  ...
  const [entitiesClient, setEntitiesClient] = useState<EntitiesApiClient | null>(null);
  const [productsClient, setProductsClient] = useState<ProductsApiClient | null>(null);
  ...
  useEffect(() => {
    const initClients = async () => {
      ...
      setEntitiesClient(new EntitiesApiClient(authClient));
      setProductsClient(new ProductsApiClient(authClient));
    };
    initClients();
  }, []);
  ...
  useEffect(() => {
    if (selectedEntity && productsClient) {
      loadEntityProducts(selectedEntity.id);
    }
  }, [selectedEntity, productsClient]);
  ...
  const loadEntityProducts = async (entityId: string) => {
    ...
    try {
      ...
      setEntityProducts(Array.isArray(data) ? data : []);
    } catch (error) {
                ...
                <div class="flex justify-between items-start mb-3">
                  <div>
                    ...
                    <div class="flex items-center gap-2 mt-1">
                      <Badge variant="soft" size="sm">{entity.type}</Badge>
                      {entity.subType && (
                        <Badge variant="soft" size="sm" color="secondary">{entity.subType}</Badge>
                      )}
                    </div>
                  </div>
                  ...
                  <div class="p-4">
                    <div class="flex justify-between items-start mb-3">
                      <div>
                        ...
                        <Badge variant="soft" size="sm" class="mt-1">{product.type}</Badge>
                      </div>
                      ...
                      <div class="mb-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                          {product.prices.map(price => (
                            <Badge key={price.id} variant="soft" color="primary">
