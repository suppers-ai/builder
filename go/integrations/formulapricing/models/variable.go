...
func GetVariableByID(id uuid.UUID) (*Variable, error) {
	query := `
		SELECT id, variable_name, display_name, description, value_type, 
		       default_value, COALESCE(is_unique, false) as is_unique, unique_behavior,
		       COALESCE(constraints, '{}') as constraints,
	...
	err := row.Scan(&v.ID, &v.VariableName, &v.DisplayName, &v.Description, &v.ValueType, 
		&v.DefaultValue, &v.IsUnique, &v.UniqueBehavior, &v.Constraints,
		&v.CreatedAt, &v.UpdatedAt)
...
func CreateVariable(v *Variable) error {
	query := `
	...
	ctx := context.Background()
	row := database.DB.QueryRow(ctx, query, v.VariableName, v.DisplayName, v.Description, v.ValueType, 
		v.DefaultValue, v.IsUnique, v.UniqueBehavior, v.Constraints)
	err := row.Scan(&v.ID, &v.CreatedAt, &v.UpdatedAt)
	if err != nil {
...
func UpdateVariable(id uuid.UUID, v *Variable) error {
	query := `
	...
	ctx := context.Background()
	row := database.DB.QueryRow(ctx, query, id, v.VariableName, v.DisplayName, v.Description, v.ValueType, 
		v.DefaultValue, v.IsUnique, v.UniqueBehavior, v.Constraints)
	err := row.Scan(&v.UpdatedAt)
	if err == sql.ErrNoRows {
		return ErrNotFound
	}
	if err != nil {
...
func DeleteVariable(id uuid.UUID) error {
