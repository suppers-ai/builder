<script lang="ts">
	...
	async function loadData() {
		try {
			...
			// Load entity details
			const entityRes = await api.get(`/user/entities/${entityId}`);
			entity = entityRes;
			...
			// Load entity's products
			const productsRes = await api.get(`/user/entities/${entityId}/products`);
	...
	async function createProduct() {
		try {
			...
			const result = await api.post(`/user/entities/${entityId}/products`, newProduct);
			if (result) {
				// Reset form
				newProduct = {
					name: '',
	...
	<div class="page-header">
		<div class="header-content">
			<div class="header-left">
				<button class="btn-icon" on:click={() => goto('/profile/products')} title="Back to Entities">
					<ArrowLeft size={20} />
				</button>
				<div>
					<div class="header-title">
						<Package size={24} />
						<h1>Products - {entity?.display_name || 'Loading...'}</h1>
					</div>
					<p class="header-subtitle">Manage products for this entity</p>
				</div>
			</div>
	...
	<div class="content-card">
		...
		{:else if filteredProducts.length === 0}
			<div class="empty-state">
				...
				{#if isAdmin}
					<p>Add your first product to this entity</p>
