...
echo "Testing Solobase Settings API..."
echo "================================"
...
if [ -z "$TOKEN" ] || [ "$TOKEN" = "null" ]; then
  ...
  sqlite3 solobase.db "SELECT email, role FROM auth_users;" 2>/dev/null || echo "   No users found in database"
  exit 1
else
  echo "SUCCESS"
...
UPDATE_RESPONSE=$(curl -s -X PATCH http://localhost:8092/api/settings \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"app_name":"Solobase Test","enable_debug_mode":true}')
