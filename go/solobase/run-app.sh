...
cleanup() {
    ...
    # Kill the solobase process if it's running
    if [ ! -z "$APP_PID" ]; then
    ...
    # Also kill any remaining solobase processes
...
export DATABASE_NAME=solobase
export DATABASE_USER=solobase
export DATABASE_PASSWORD=solobase123
export DATABASE_SSL_MODE=disable

export S3_ENDPOINT=http://localhost:9002
export S3_ACCESS_KEY=minioadmin
export S3_SECRET_KEY=minioadmin123
export S3_BUCKET=solobase
export S3_REGION=us-east-1
export S3_USE_SSL=false

export SMTP_HOST=localhost
export SMTP_PORT=1026
export SMTP_FROM=noreply@solobase.local
