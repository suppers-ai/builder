...
func HandleGetLogs(logsService *services.LogsService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			RespondWithError(w, http.StatusInternalServerError, "Failed to fetch logs")
			return
		}
...
func HandleGetLogStats(logsService *services.LogsService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			RespondWithError(w, http.StatusInternalServerError, "Failed to fetch log statistics")
			return
		}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			if reqLog.RequestBody != nil {
				response["requestBody"] = *reqLog.RequestBody
			}
			if reqLog.ResponseBody != nil {
				response["responseBody"] = *reqLog.ResponseBody
			}
			if reqLog.Headers != nil {
				response["headers"] = *reqLog.Headers
			}
